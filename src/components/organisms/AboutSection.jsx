...
export function AboutSection({ onDownloadCV }) {
  ...
  const skillsText = info?.skills && info.skills.length > 0 
    ? info.skills.map(s => s.toUpperCase()).join(' ') 
    : "SQL MYSQL SUPABASE GSAP WEB DESIGN UI/UX ESP32 MICROCONTROLLER";

  return (
    <section id="about" className="py-16 md:py-24 hairline-t scroll-fade bg-white flex flex-col justify-between">
      <div className="w-full">
        <SectionHeader number="01" title={t('about.title')} />
        
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 px-6 md:px-12 items-start mb-16">
